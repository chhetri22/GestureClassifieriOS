...
public class KNNDTW_3D: NSObject {
    ...
    private var curve_label_pairs_3d: [knn_curve_label_pair_3d] = [knn_curve_label_pair_3d]()
    ...
    public func train(data_sets: [knn_curve_label_pair_3d]) {
        
        
        self.curve_label_pairs_3d = data_sets
    ...
    public func predict(curve_to_test_x: [Float], curve_to_test_y: [Float], curve_to_test_z: [Float]) -> knn_certainty_label_pair_3d {
        ...
        return knn_certainty_label_pair_3d(probability: Float(sorted_votes!.1)/Float(self.n_neighbors), label: (sorted_votes?.0)!)
...
//input type
public struct knn_curve_label_pair_3d {
    let curveAccX: [Float]
    let curveAccY: [Float]
    let curveAccZ: [Float]
    let label: String
...
//output type
public struct knn_certainty_label_pair_3d {
